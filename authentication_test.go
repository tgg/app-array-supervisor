...
func TestNewVaultAuthenticationManager(t *testing.T) {
	manager := NewVaultAuthenticationManager("http://localhost:8200", "dev-root-token", "kv", "password")
	assert.True(t, manager != nil)
}

func TestNewVaultAuthenticationManagerWithIncorrectPath(t *testing.T) {
	manager := NewVaultAuthenticationManager("http://localhost:8200", "dev-root-token", "kvv", "password")
	assert.True(t, manager == nil)
}

func TestNewVaultAuthenticationManagerWithIncorrectToken(t *testing.T) {
	manager := NewVaultAuthenticationManager("http://localhost:8200", "false-token", "kv", "password")
	assert.True(t, manager == nil)
}

func TestNewVaultAuthenticationManagerWithIncorrectHost(t *testing.T) {
	manager := NewVaultAuthenticationManager("http://localhost:8201", "dev-root-token", "kv", "password")
	assert.True(t, manager == nil)
}
...
func TestVaultAuthenticationManager_GetCredentialsUnknownLogin(t *testing.T) {
	manager := NewVaultAuthenticationManager("http://localhost:8200", "dev-root-token", "kv", "secret")
